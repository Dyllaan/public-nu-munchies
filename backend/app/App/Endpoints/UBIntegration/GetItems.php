 ...
 * fetches all items that are available within the next 24 hours that aren't already reserved, along with the necessary data.
 */
...
class GetItems extends Endpoint
{
    ...
    public function process($request)
    {
        ...
        $id = $this->getDb()->createSelect()
            ->from('items')
            ->cols("items.id, items.business_id, item_name, item_price, TO_CHAR(collect_time, 'HH24:MI') as collect_time, TO_CHAR(collect_time, 'DD') as collect_date, ROUND(COALESCE(AVG(rating), 0.0), 1) 
    AS average_rating, businesses.business_name, businesses.business_description, addresses.address_line1, addresses.address_postcode, addresses.address_city, weight, calories, protein, carbs, fat, salt, quantity")
